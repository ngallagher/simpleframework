...
/**
 * The <code>ExecutorQueue</code> object is used to queue tasks in a thread
 * pool. This creates a thread pool with no limit to the number of tasks that
 * can be enqueued, which ensures that any system requesting a task to be
 * executed will not block when handing it over, it also means the user must use
 * caution.
 *
...
class ExecutorQueue {
    ...
    /**
     * Constructor for the <code>ExecutorQueue</code> object. This is used to
