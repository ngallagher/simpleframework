...
public class MockALPNServer implements Container {   
   ...
   public static void main(String[] list) throws Exception {
      ...
      File file = new File("C:\\Work\\development\\bitbucket\\proxy\\zuooh-standard-proxy-site\\certificate\\www.zuooh.com.pfx");
      if(!file.exists()) {
         throw new IOException("Could not find PFX file");
      }
      KeyStoreReader reader = new KeyStoreReader(KeyStoreType.PKCS12, file, "password12", "password12");
      SecureSocketContext context = new SecureSocketContext(reader, SecureProtocol.TLS);
      SSLContext sslContext = context.getContext();      
      MockALPNServer server = new MockALPNServer(sslContext, false, 10001);      
      server.start();
   }

   public MockALPNServer(SSLContext context, boolean certRequired, int port) throws IOException {
