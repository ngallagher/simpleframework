...
/**
 * The <code>Message</code> object is used to store an retrieve the headers for
 * both a request and response. Headers are stored and retrieved in a case
 * insensitive manner according to RFC 2616. The message also allows multiple
 * header values to be added to a single header name, headers such as Cookie and
 * Set-Cookie can be added multiple times with different values.
 *
...
public class MessageHeader implements Message {
    ...
    /**
     * Constructor for the <code>Message</code> object. This is used to create a
     * case insensitive means for storing HTTP header names and values. Dates
     * can also be added to message as a long value and is converted to RFC 1123
    ...
    /*
   TODO:
   keep charset in content-type if already set
     */
   ...
   public ContentType getContentType() {
    ...
    public String buildContentType(String type) {
        if (!type.toLowerCase().contains("charset")) {
                        
            if (getContentType() == null) {
                ContentTypeParser parser = new ContentTypeParser();
                type += "; charset=" + parser.getCharset();
            } else {
                type += "; charset=" + getContentType().getCharset();
            }
        }
        return type;
    }
    ...
    /**
     * This can be used to get the value of the first message header that has
     * the specified name. This will return the long representing the named
     * header value. If the named header does not exist then this will return a
     * value of minus one, -1.
     *
     * @param name the HTTP message header to get the value from
    ...
    /**
     * This is used to acquire all the individual header values from the
     * message. The header values provided by this are unparsed and represent
     * the actual string values that have been added to the message keyed by a
     * given header name.
     *
    ...
    /**
     * The <code>Series</code> object is used to represent a list of HTTP header
     * value for a given name. It allows multiple values to exist for a given
     * header, such as the Cookie header. Most entries will contain a single
     * value.
     */
    private class Series {
        ...
        /**
         * Constructor for the <code>Entry</code> object. The entry is created
         * using the name of the HTTP header. Values can be added to the entry
         * list in order to build up the header.
         */
        public Series() {
        ...
        /**
         * This returns the list of header values associated with the header
         * name. Each value is added as an individual header prefixed by the
         * header name and a semicolon character.
         *
