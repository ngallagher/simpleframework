...
public class SecureSocketConnector extends SocketConnector {
   ...
   private final KeyManagerFactory keyFactory;
   private final SSLContext context;
   private final SocketFactory factory;   
   private final InputStream certificate;
   
   public SecureSocketConnector() throws Exception {
      this.store = KeyStore.getInstance("JKS");      
      this.keyFactory = KeyManagerFactory.getInstance("SunX509");
      this.context = SSLContext.getInstance("TLS");
      this.trustManager = new AnonymousTrustManager();
      this.trustManagers = new X509TrustManager[]{trustManager};
      this.certificate = new ByteArrayInputStream(CERTIFICATE);
      this.store.load(certificate, "password".toCharArray());
      this.keyFactory.init(store, "password".toCharArray());
      this.context.init(keyFactory.getKeyManagers(), trustManagers, null);      
