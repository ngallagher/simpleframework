...
function reportStatus(socket, status, height, delta, change, duration, sequence) {
	...
	socket.send("status:rows="+height+",change="+change+",duration="+duration+",sequence="+sequence);
...
function deltaUpdate(socket, table, message) {	
	...
	reportStatus(socket, "success.png", height, length, change, duration, sequence);
}
...
function interpolateRow(record, template) {
	for ( var i = 0; i < schema.length; i++) {
		...
		template.style[i] = interpolateCell(record, template, column, style);
		template[name] = interpolateCell(record, template, column, text);
	}
}

function interpolateCell(record, template, column, text) {
	var name = column.name;	
