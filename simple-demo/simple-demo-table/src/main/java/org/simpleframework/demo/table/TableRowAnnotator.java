...
public class TableRowAnnotator {
   
   private final TableSchema schema;
   private final TableValueEncoder encoder;
   
   public TableRowAnnotator(TableSchema schema) {
      this.encoder = new TableValueEncoder();
      this.schema = schema;
   }

   public String calculateHighlight(TableRow row, long lastUpdateDone) {      
      ...
      for(int i = 0; i < columns.size(); i++){
         ...
         TableCell cell = row.getValue(column);
